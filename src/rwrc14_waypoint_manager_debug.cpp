...
{
	string waypoint_file_name("/home/amsl/AMSL_ros_pkg/rwrc14/rwrc14_waypoint_manager/lat_lon_data/");
	if(init_state.LocateFlag==0){// if flag==0 --> ikuta file
		switch(init_state.course_num){
			...
			case 4:	
				...
				break;
			case 5:	
				waypoint_file_name += "jrm15_5_corrected.txt";//<------------------0210変更
				break;
			default:
				break;
		}
...
int main(int argc, char** argv){
	...
	ros::init(argc, argv, "rwrc14_waypoint_manager_debug");
	ros::NodeHandle n;

	cout<<"waypoint server"<<endl;	

	ros::Publisher marker_pub = n.advertise<visualization_msgs::Marker>("localization/waypoints_", 100);
	ros::Publisher t_circle_pub = n.advertise<visualization_msgs::Marker>("localization/traced_waypoint_circle_", 100);
	//ros::Publisher marker_pub2 = n.advertise<visualization_msgs::Marker>("localization/waypoints/dr", 100);
	ros::Publisher marker_pub4 = n.advertise<visualization_msgs::MarkerArray>("localization/waypoints/numaa", 100);
	marker_pub5 = n.advertise<visualization_msgs::Marker>("/waypoint/aav", 100);
	ros::Publisher inter_pub = n.advertise<geometry_msgs::PoseStamped>("/aawaypoint/inter", 100);
	ros::Publisher dist_pub = n.advertise<std_msgs::Float32>("/waypoint/disaatance", 1);
	ros::Publisher num_pub = n.advertise<std_msgs::Int32>("/waypoint/noaaw", 1);
	ros::Publisher poi_pub = n.advertise<sensor_msgs::PointCloud>("/waypoaaint/point_cloud", 1);
	ros::Publisher target_pub = n.advertise<geometry_msgs::PoseStamped>("/taaarget/pose", 100);
	ros::Publisher prev_wp_pub = n.advertise<geometry_msgs::PoseStamped>("/wayaapoint/prev", 100);
	carrot_pub = n.advertise<geometry_msgs::PoseStamped>("/waypoint/carraaaot", 100);
